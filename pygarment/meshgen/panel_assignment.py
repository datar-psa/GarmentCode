...
def create_face_filter(_body_verts, _body_indices, _body_seg, parts, smpl_body=False):
    """Create a filter that excludes faces belonging to indicated body parts on a given mesh."""

    body_seg = process_body_seg(_body_seg, smpl_parts=smpl_body)

    body_vert_labels = ["" for _ in range(len(_body_verts))]
    for part, verts in body_seg.items():
        for vert in verts:
            # NOTE: Vertex could have multiple labels -- the last one is assigned
    ...
    for i in range(int(len(_body_indices) / 3)):
        ...
        part = [0, 0, 0]
        tri_filter = [False, False, False]

        part[0], part[1], part[2] = (
            body_vert_labels[tri[0]],
            body_vert_labels[tri[1]],
            body_vert_labels[tri[2]],
        )

        # NOTE: Small number of vertices may not have a label: they won't be filtered.
        tri_filter[0], tri_filter[1], tri_filter[2] = (
            part[0] in parts,
            part[1] in parts,
            part[2] in parts,
        )
        ...
        if tri_filter[0] == tri_filter[1] or tri_filter[0] == tri_filter[2]:
            face_filter.append(tri_filter[0])
        else:
            face_filter.append(tri_filter[1])
    ...
    * current_vertex_filter -- existing filter assignments on vertices
    ...
    for i, label in enumerate(panel_verts_label):
        if label in parts:
            ...
            neighbours = vert_connectivity[i]
            n_labels = [panel_verts_label[n] for n in neighbours]
...
def panel_assignment(
    ...
    * merge_two_legs: Assign 'body' label to panels that should be labeled with one of the legs,
    ...
    for index, verts in enumerate(body_seg.values()):
        for vert in verts:
    ...
    for p in panels:
        ...
        if strategy == "ray_hit":
            ...
            results = _count_closest_hits(p_verts, body_shape.id, body_transform, device=device)
        else:
        ...
        for hit in results.numpy():
            if hit == -1:
                # No hit
                continue
            # Each vertex on the body can belong to multiple body parts
            f1, f2, f3 = (
                body_indices[hit * 3 + 0],
                body_indices[hit * 3 + 1],
                body_indices[hit * 3 + 2],
            )
...
def _count_closest_hits(p_verts, body_id, body_transform, device):
    ...
    wp.launch(
        ...
        dim=len(p_verts),
        inputs=[body_id, body_transform, p_verts],
        outputs=[results],
        device=device,
    )
    return results
...
def _count_ray_hits(panel_verts, panel_indices, p_vert_index, p_verts, body_id, body_transform, device):
    ...
    wp.launch(
        ...
        dim=len(p_verts),
        inputs=[body_id, body_transform, p_verts, normal],
        outputs=[results],
        device=device,
    )
